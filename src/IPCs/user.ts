...
const userIPCs = (): void => {
    ...
    ipcMain.handle(constants.USER_GET, (event: IpcMainInvokeEvent, { id }: { id: string }): Promise<IPCUserInterface> => services.get({ id }));
    ...
    ipcMain.handle(constants.USER_DELETE, (event: IpcMainInvokeEvent, { isSelected }: { isSelected: boolean }): Promise<IPCUserInterface> => services.selectAll({ isSelected }));
